...
s.linkInternalFilters="javascript:,ajc.healthology.com,stats.com,mundohispanico.com,placead.advertise123.com,ajcclassifieds.com,legacy.com,ajcsportsplus.com,ajcjobs.com,ajchomefinder.com,ajccars.com,accessatlanta.com,ajc.com,ajc.stockgroup.com,prnewswire.com,commerce.coxnewsweb.com,harvestadsdepot.com,newspaperads.com,newsbank.com,newslibrary.com,print2webcorp.com,sportsbuff.com,shopping.ajc.com,formrouter.net,stubhub.com,uclick.com,upuzzles.com,mycomicspage.com,cityfeet.com,salary.com,wetfeet.com,ajcclasifieds.com,ajcpets.com,ajcstuff.com,hosted.ap.org,clickability.com,coxnewsweb.com,ap.org,localwireless.com,ajcmobile.com,reservetravel.com,nieonline.com,adpay.com,advertise123.com,bigprofootball.com,coxohio.com,coxohiointeractive.com,digitalmediaclassifieds.com,gabriels.com,interest.com,marketplacelocal.com,morris.com,morrisdigitalworks.com,mtgeinfo.com,newhomesource.com,statesmanapartmentsearch.com,western-star.com,zap2it.com,pqarchiver.com,169.137.232.87,ajc4me.com,gabriels.net,ajcexchange.com,ajcshopping.com,eveningedge.com,ajcprint.com,outbrain.com";
...
/********************************************************************
 *
 * Channel Manager Config variables (should be above doPlugins section)
 *
 ********************************************************************
    s._extraSearchEngines - Extra Search Engines.  This variable is used to list the additional Search Engine checks, that
        ...
        Example: If Microsoft Bing were to change it's name to Microsoft Zing, then the following would be my s._extraSearchEngines:
...
s._channelParameter="SEO|seo";

/* Site Search Config Variables */
s.successfulSearchEvent = 'event13';
s.nullSearchEvent = 'event14';
...
/* Plugin: channelManager v2.4 */
    s.channelManager('seo,cmpid');
    s.eVar58=s._channel;
    s.eVar44=s._keywords;

/* Set Page View Event */
    s.events=s.apl(s.events,'event1',',',2);

/* Set Days since Last Visit */
    s.eVar18=s.prop18=s.getDaysSinceLastVisit('s_lv').toLowerCase();

/* Set New vs. Repeat */
    s.eVar19=s.prop19 = s.getNewRepeat().toLowerCase();

/* Plugin: getVisitNum 3.0 */
    s.eVar20=s.prop20=s.getVisitNum(365);

/* Plugin: getPercentPageView v1.2 */
    s.eVar47=s.prop47=s.getPercentPageViewed();

/* Plugin: getPreviousValue v1.0 */

    s.eVar57 = s.prop57 = s.getPreviousValue(s.pageName,'gpv_p57','');

/* Set home page defaults */
    if (s.pageName == "/"){
       cmg.s_coxnews.pageName = cmg.s_coxnews.eVar55 = s.eVar55 = s.pageName =  "/homepage/";
    }

/* Site Search */
    if(!s.eVar1) {
        s.eVar1=s.getQueryParam('q');
        s.prop44=s.eVar1=s.eVar1.toLowerCase();
    }
...
/* Internal Campaigns */
    if(!s.eVar15) {
        s.eVar15 = s.getQueryParam('icmp');
    }

/*socialPlatforms v1.0 - used for SocialAnalytics*/
	s.socialPlatforms('eVar38');


/* External Campaigns */
/* if campaign doesn't have a value, get ecmp from the query string */
    if(!s.campaign) {
        s.campaign = s.getQueryParam('ecmp');
    }
}
...
+"'SocialAuthor');s.Integrate.SocialAuthor.tEvar='eVar23';s.Integrate"
...
s.trackingServer="coxnet.112.2o7.net"
